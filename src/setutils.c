...
int index_set_types[] = {SET_XY,   SET_XYDX, SET_XYDY,  SET_XYDXDX,    SET_XYDYDY,   SET_XYDXDY, SET_XYZ, SET_XYHILO,
                         SET_XYRT, SET_XYUV, SET_XYBOX, SET_XYBOXPLOT, SET_XYSTRING, SET_POLY,   -1};
int index_set_ncols[] = {2, 3, 3, 4, 4, 4, 3, 5, 3, 4, 5, 2, 3, -1};
...
void getminmaxall(int gno, int setno) {
    ...
    for (i = 0; i < ncols; i++) {
        ...
        if (n == 0) {
            ...
            minmax(x, n, &g[gno].p[setno].emin[i], &g[gno].p[setno].emax[i], &g[gno].p[setno].imin[i],
...
void* geteditpoints(int gno, int setno) { return g[gno].p[setno].ep; }
...
void copycol(int gno, int setfrom, int setto, int col) { copycol2(gno, setfrom, gno, setto, col); }
 ...
 */
void do_packsets(void) { packsets(cg); }
...
int nextset(int gno) {
    ...
    if ((target_set.gno == gno) && (target_set.setno >= 0) && (target_set.setno < g[gno].maxplot) &&
        !isactive_set(gno, target_set.setno) && !g[gno].p[target_set.setno].deact) {
        i = target_set.setno;
        target_set.gno = -1;
        target_set.setno = -1;
        return (i);
...
void findpoint(int gno, double x, double y, double* xs, double* ys, int* setno, int* loc) {
    double dx = g[gno].w.xg2 - g[gno].w.xg1, dy = g[gno].w.yg2 - g[gno].w.yg1, *xtmp, *ytmp, tmp, tmin = MAXNUM;
    int i, j, len;
...
int is_hotlinked(int gno, int setno) { return (g[gno].p[setno].hotlink && strlen(g[gno].p[setno].hotfile) > 0); }

char* get_hotlink_file(int gno, int setno) { return g[gno].p[setno].hotfile; }

int get_hotlink_src(int gno, int setno) { return g[gno].p[setno].hotsrc; }
...
void set_work_pending(int d) { wp = d; }

int work_pending(void) { return wp; }

static int dp = 0;

void set_lists_dirty(int d) { dp = d; }

int lists_dirty(void) { return dp; }
