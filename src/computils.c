...
static void seasonaldiff(double* x, double* y, double* resx, double* resy, int n, int period) {
    int i;
...
void do_digfilter(int set1, int set2) {
    ...
    if (digfiltset != (-1)) {
        ...
        filterser(getsetlength(cg, set1), getx(cg, set1), gety(cg, set1), getx(cg, digfiltset), gety(cg, digfiltset),
                  gety(cg, set2), getsetlength(cg, set2));
...
void do_linearc(int set1, int set2) {
    ...
    if (linearcset != (-1)) {
        ...
        linearconv(gety(cg, set2), gety(cg, set1), gety(cg, linearcset), getsetlength(cg, set2),
                   getsetlength(cg, set1));
...
void do_xcor(int set1, int set2, int maxlag) {
    ...
    if (xcorset != (-1)) {
        ...
        if (set1 != set2) {
            sprintf(buf, "X-correlation of set %d and %d at maximum lag %d", set1, set2, maxlag);
        } else {
            sprintf(buf, "Autocorrelation of set %d at maximum lag %d", set1, maxlag);
        }
        ierr = crosscorr(gety(cg, set1), gety(cg, set2), getsetlength(cg, set1), maxlag, getx(cg, xcorset),
                         gety(cg, xcorset));
...
double do_int(int setno, int itype) {
    ...
    if (itype == 0) {
        ...
        if (intset != (-1)) {
            ...
            sum =
                trapint(getx(cg, setno), gety(cg, setno), getx(cg, intset), gety(cg, intset), getsetlength(cg, setno));
...
void do_differ(int setno, int itype) {
    ...
    if (diffset != (-1)) {
        ...
        case 0:
            ...
            forwarddiff(getx(cg, setno), gety(cg, setno), getx(cg, diffset), gety(cg, diffset),
                        getsetlength(cg, setno));
            break;
        case 1:
            sprintf(buf, "Backward difference of set %d", setno);
            setlength(cg, diffset, getsetlength(cg, setno) - 1);
            backwarddiff(getx(cg, setno), gety(cg, setno), getx(cg, diffset), gety(cg, diffset),
                         getsetlength(cg, setno));
            break;
        case 2:
            sprintf(buf, "Centered difference of set %d", setno);
            setlength(cg, diffset, getsetlength(cg, setno) - 2);
            centereddiff(getx(cg, setno), gety(cg, setno), getx(cg, diffset), gety(cg, diffset),
                         getsetlength(cg, setno));
            break;
        }
...
void do_seasonal_diff(int setno, int period) {
    ...
    if (diffset != (-1)) {
        ...
        seasonaldiff(getx(cg, setno), gety(cg, setno), getx(cg, diffset), gety(cg, diffset), getsetlength(cg, setno),
                     period);
...
void apply_window(double* xx, double* yy, int ilen, int type, int wind) {
    ...
    for (i = 0; i < ilen; i++) {
        ...
        case 5: /* Blackman */
            if (type != 0) {
                xx[i] = xx[i] *
                        (0.42 - 0.5 * cos(2.0 * M_PI * i / (ilen - 1.0)) + 0.08 * cos(4.0 * M_PI * i / (ilen - 1.0)));
            }
            yy[i] =
                yy[i] * (0.42 - 0.5 * cos(2.0 * M_PI * i / (ilen - 1.0)) + 0.08 * cos(4.0 * M_PI * i / (ilen - 1.0)));
            break;
...
void do_prune(int setno, int typeno, int deltatypeno, char* dxstr, char* dystr, int dxtype, int dytype) {
    ...
    case PRUNE_RECTANGLE:
        scanner(dxstr, x, y, len, ax, bx, cx, dx, MAXARR, 0, setno, &errpos);
        if (errpos) {
            return;
        }
        deltax = fabs(result);
    ...
    case PRUNE_INTERPOLATION:
        scanner(dystr, x, y, len, ax, bx, cx, dx, MAXARR, 0, setno, &errpos);
        if (errpos) {
            return;
        }
        deltay = fabs(result);
...
sets */
    ...
    if (method) { /* spline */
        ...
        for (i = 0; i < j; i++) {
            add_point(cg, iset, newx[i], seval(ysetlength, newx[i], x1, y, b, c, d), 0.0, 0.0, SET_XY);
        }
        ...
        for (j = 0; j < isetlength; j++) {
            ...
            if (i >= ysetlength)
                ...
                newy = (newx[j] - x1[i]) * (y[i + 1] - y[i]) / (x1[i + 1] - x1[i]) + y[i];
