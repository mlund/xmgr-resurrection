...
void create_printer_setup(Widget w, XtPointer client_data, XtPointer call_data) {
    ...
    if (psetup_frame == NULL) {
        ...
        devices_item = CreatePanelChoice(pdev_rc, "Device: ", 12, "PostScript landscape", "PostScript portrait",
                                         "FrameMaker landscape", "FrameMaker portrait", "HPGL landscape",
                                         "HPGL portrait", "Interleaf landscape", "Interleaf portrait", "PSTeX include",
                                         "PSTeX landscape", "PSTeX portrait", 0, 0);
        for (i = 0; i < 11; i++) {
            XtAddCallback(devices_item[2 + i], XmNactivateCallback, (XtCallbackProc)do_prstr_toggle,
                          (XtPointer)(intptr_t)i);
        }
        printto_item = CreatePanelChoice(pdev_rc, "Print to: ", 3, "Printer", "File", 0, 0);
        for (i = 0; i < 2; i++) {
            XtAddCallback(printto_item[2 + i], XmNactivateCallback, (XtCallbackProc)do_pr_toggle,
                          (XtPointer)(intptr_t)i);
        }
        printstring_item = CreateTextItem2(psetup_rc, 30, "Print control string:");
        ...
        pslw_rc = XtVaCreateWidget("pslw_rc", xmRowColumnWidgetClass, psetup_rc, XmNorientation, XmHORIZONTAL, NULL);

        pslwbegin_item = CreatePanelChoice(pslw_rc, "PS line width start: ", 10, "1", "2", "3", "4", "5", "6", "7", "8",
                                           "9", NULL, NULL);
        pslwincr_item = CreatePanelChoice(pslw_rc, "PS line width increment: ", 10, "1", "2", "3", "4", "5", "6", "7",
                                          "8", "9", NULL, NULL);

        XtVaCreateManagedWidget("sep", xmSeparatorWidgetClass, psetup_rc, NULL);

        ps_rc = XtVaCreateWidget("ps_rc", xmRowColumnWidgetClass, psetup_rc, XmNorientation, XmHORIZONTAL, XmNspacing,
                                 80, NULL);

        eps_item = XtVaCreateManagedWidget("Generate EPS ", xmToggleButtonWidgetClass, ps_rc, NULL);
        XtAddCallback(eps_item, XmNvalueChangedCallback, (XtCallbackProc)do_eps_toggle, NULL);
        ps2_item = XtVaCreateManagedWidget("Level 2 PS ", xmToggleButtonWidgetClass, ps_rc, NULL);

        XtVaCreateManagedWidget("sep", xmSeparatorWidgetClass, psetup_rc, NULL);

        CreateCommandButtons(psetup_rc, 5, buts, label1);
        XtAddCallback(buts[0], XmNactivateCallback, (XtCallbackProc)set_printer_proc, (XtPointer)NULL);
        XtAddCallback(buts[1], XmNactivateCallback, (XtCallbackProc)do_print, (XtPointer)NULL);
        XtAddCallback(buts[2], XmNactivateCallback, (XtCallbackProc)create_printfiles_popup, (XtPointer)NULL);
        XtAddCallback(buts[3], XmNactivateCallback, (XtCallbackProc)destroy_dialog, (XtPointer)psetup_frame);
        XtAddCallback(buts[4], XmNactivateCallback, (XtCallbackProc)HelpCB, (XtPointer) "file.html#printersetup");
...
static void do_prfilesel_proc(Widget w, XtPointer client_data, XtPointer call_data) {
...
void create_printfiles_popup(Widget w, XtPointer client_data, XtPointer call_data) {
    ...
    if (!GetChoice(printto_item)) {
        ...
        if (top == NULL) {
            ...
            XtAddCallback(top, XmNokCallback, (XtCallbackProc)do_prfilesel_proc, (XtPointer)top);
            XtAddCallback(top, XmNcancelCallback, (XtCallbackProc)destroy_dialog, (XtPointer)top);
        }
        XtVaSetValues(top, XmNdirMask, XmStringConcat(XmStringCreate("*", charset), XmStringCreate(cursuffix, charset)),
                      NULL);
        XtRaise(top);
        unset_wait_cursor();
    }
