...
static int gtypes[] = {GRAPH_XY,   GRAPH_LOGX,        GRAPH_LOGY,  GRAPH_LOGXY, GRAPH_BAR, GRAPH_STACKEDBAR,
                       GRAPH_HBAR, GRAPH_STACKEDHBAR, GRAPH_POLAR, GRAPH_SMITH, GRAPH_XY,  GRAPH_XY};
...
static void update_focus_items(int gno) {
    ...
    if (graph_focus_frame) {
        ...
        XmToggleButtonSetState(graph_drawfocus_choice_item, draw_focus_flag == TRUE ? True : False, False);
    }
}
...
void create_gfocus_frame(Widget w, XtPointer client_data, XtPointer call_data) {
    ...
    if (graph_focus_frame == NULL) {
        ...
        graph_focus_choice_item =
            (Widget*)CreatePanelChoice(dialog, "Graph focus", 4, "Button press", "As set", "Follows mouse", NULL, NULL);
        graph_drawfocus_choice_item =
            XtVaCreateManagedWidget("Display focus markers", xmToggleButtonWidgetClass, dialog, NULL);
...
void create_gshow_frame(Widget w, XtPointer client_data, XtPointer call_data) {
    ...
    if (graph_show_frame == NULL) {
        ...
        lab = XtVaCreateManagedWidget(
            "Select graphs for display (a graph must also be active to be seen):", xmLabelWidgetClass, dialog, NULL);
        rc = XtVaCreateManagedWidget("rc", xmRowColumnWidgetClass, dialog, XmNorientation, XmHORIZONTAL, XmNnumColumns,
                                     maxgraph / 10, XmNpacking, XmPACK_COLUMN, NULL);
        graph_show_choice_item = (Widget*)XtMalloc(maxgraph * sizeof(Widget));
        for (i = 0; i < maxgraph; i++) {
            sprintf(buf, "%d", i);
            graph_show_choice_item[i] = XtVaCreateManagedWidget(buf, xmToggleButtonWidgetClass, rc, NULL);
        }
...
void create_gtype_frame(Widget w, XtPointer client_data, XtPointer call_data) {
    ...
    if (graph_type_frame == NULL) {
        ...
        graph_rendsets_choice_item = CreatePanelChoice(
            dialog, "Set current graph type to", 11, "XY graph", "Log-linear", "Linear-log", "Log-log", "Bar chart",
            "Stacked bar", "Horizontal bar chart", "Horizontal stacked bar", "Polar chart", "Smith chart", NULL, NULL);
...
void close_rimage_popup(Widget w, XtPointer client_data, XtPointer call_data) { XtUnmanageChild(rimage_dialog); }
...
void read_image(char* fname) {
    ...
    set_wait_cursor();
    img = read_image_from_disk(disp, xwin, fname, &width, &height, &depth);
    unset_wait_cursor();
    if (img != NULL) {
        imagew = width;
        imageh = height;
...
void do_rimage_proc(Widget w, XtPointer client_data, XtPointer call_data) {
    ...
    set_wait_cursor();
    img = read_image_from_disk(disp, xwin, image_filename, &width, &height, &depth);
    if (img != NULL) {
        imagew = width;
        imageh = height;
...
void create_ginvert_frame(Widget w, XtPointer client_data, XtPointer call_data) {
    ...
    if (graph_invert_frame == NULL) {
        ...
        graph_invertx_item = XtVaCreateManagedWidget("Invert X", xmToggleButtonWidgetClass, dialog, NULL);
        graph_inverty_item = XtVaCreateManagedWidget("Invert Y", xmToggleButtonWidgetClass, dialog, NULL);
        graph_flipxy_item = XtVaCreateManagedWidget("Flip GRAPH_XY", xmToggleButtonWidgetClass, dialog, NULL);
        graph_invert_choice_item = CreatePanelChoice(dialog, "Apply to graph", 3, "Current", "All", NULL, NULL);
