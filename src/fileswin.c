...
void create_file_popup(Widget wid, XtPointer client_data, XtPointer call_data) {
    ...
    if (rdata_dialog == NULL) {
        ...
        XtAddCallback(rdata_dialog, XmNhelpCallback, (XtCallbackProc)HelpCB, (XtPointer) "file.html#readsets");
        ...
        read_ftype_item =
            CreatePanelChoice(rc2, "File format: ", 18, "X Y", "X Y1 Y2 ... ", "IHL", "Binary", "X Y DX", "X Y DY",
                              "X Y DX1 DX2", "X Y DY1 DY2", "X Y DX DY", "X Y Z", "X HI LO OPEN CLOSE", "X Y RADIUS",
                              "X Y BOX", "Rawspice", "X Y BOXPLOT", "X Y String", "POLY", NULL, NULL);
...
void create_rparams_popup(Widget w, XtPointer client_data, XtPointer call_data) {
    ...
    if (rparams_dialog == NULL) {
        ...
        XtAddCallback(rparams_dialog, XmNhelpCallback, (XtCallbackProc)HelpCB, (XtPointer) "file.html#readpars");
        xv_setstr(tw, plfile);
    }
...
void create_wparam_frame(Widget w, XtPointer client_data, XtPointer call_data) {
    ...
    if (wparam_frame == NULL) {
        ...
        XtAddCallback(wparam_frame, XmNhelpCallback, (XtCallbackProc)HelpCB, (XtPointer) "file.html#writeparams");
...
void create_workingdir_popup(Widget w, XtPointer client_data, XtPointer call_data) {
    ...
    if (workingd_dialog == NULL) {
        ...
        XtAddCallback(workingd_dialog, XmNhelpCallback, (XtCallbackProc)HelpCB, (XtPointer) "options.html#workdir");
    ...
    XtVaSetValues(workingd_dialog, XmNdirectory, str = XmStringCreateLtoR(workingdir, charset), NULL);
    XmFileSelectionDoSearch(workingd_dialog, NULL);
    XmStringFree(str);
...
static void do_netcdf_proc(Widget w, XtPointer client_data, XtPointer call_data) {
    ...
    if (XmListGetSelectedPos(netcdf_listx_item, &pos_list, &pos_cnt)) {
        XtVaGetValues(netcdf_listx_item, XmNselectedItemCount, &cnt, XmNselectedItems, &s, NULL);
        cs = XmStringCopy(*s);
    ...
    if (XmListGetSelectedPos(netcdf_listy_item, &pos_list, &pos_cnt)) {
        j = pos_list[0];
        XtVaGetValues(netcdf_listy_item, XmNselectedItemCount, &cnt, XmNselectedItems, &s, NULL);
        cs = XmStringCopy(*s);
        if (XmStringGetLtoR(cs, charset, &cstr)) {
            strcpy(yvar, cstr);
            XtFree(cstr);
...
void create_netcdfs_popup(Widget w, XtPointer client_data, XtPointer call_data) {
    ...
    if (top == NULL) {
        ...
        */
        ...
        XtAddCallback(but1[0], XmNactivateCallback, (XtCallbackProc)do_netcdf_proc, (XtPointer)NULL);
        XtAddCallback(but1[1], XmNactivateCallback, (XtCallbackProc)create_netcdffiles_popup, (XtPointer)NULL);
        XtAddCallback(but1[2], XmNactivateCallback, (XtCallbackProc)do_netcdfupdate_proc, (XtPointer)NULL);
        XtAddCallback(but1[3], XmNactivateCallback, (XtCallbackProc)do_netcdfquery_proc, (XtPointer)NULL);
...
void create_saveproject_popup(void) {
    ...
    if (sui.top == NULL) {
        ...
        XtAddCallback(sui.top, XmNhelpCallback, (XtCallbackProc)HelpCB, (XtPointer) "file.html#save");
...
void create_openproject_popup(void) {
    ...
    if (oui.top == NULL) {
        ...
        XtAddCallback(oui.top, XmNhelpCallback, (XtCallbackProc)HelpCB, (XtPointer) "file.html#open");
    }
...
void create_describe_popup(Widget w, XtPointer client_data, XtPointer call_data) {
    ...
    if (describe_frame == NULL) {
        ...
        XtVaSetValues(text_w, XmNrows, 10, XmNcolumns, 80, XmNeditMode, XmMULTI_LINE_EDIT, XmNfontList, xmf,
                      XmNwordWrap, True, NULL);
        XtManageChild(text_w);

        XtVaCreateManagedWidget("sep", xmSeparatorWidgetClass, describe_panel, NULL);
        CreateCommandButtons(describe_panel, 2, buts, label);
        XtAddCallback(buts[0], XmNactivateCallback, (XtCallbackProc)update_descript, (XtPointer)text_w);
        XtAddCallback(buts[1], XmNactivateCallback, (XtCallbackProc)destroy_dialog, (XtPointer)describe_frame);
...
void create_write_popup(Widget w, XtPointer client_data, XtPointer call_data) {
    ...
    if (wui.top == NULL) {
        ...
        XtAddCallback(wui.top, XmNhelpCallback, (XtCallbackProc)HelpCB, (XtPointer) "file.html#writesets");

        fr = XmCreateFrame(wui.top, "fr", NULL, 0);
        dialog = XmCreateRowColumn(fr, "dialog_rc", NULL, 0);

        wui.sel = CreateSetSelector(dialog, "Write set:", SET_SELECT_ALL, FILTER_SELECT_NONE, GRAPH_SELECT_CURRENT,
                                    SELECTION_TYPE_MULTIPLE);
        wui.graph_item = CreateGraphChoice(dialog, "From graph:", maxgraph, 2);
        wui.embed_item = XtVaCreateManagedWidget("Embed parameters", xmToggleButtonWidgetClass, dialog, NULL);
        wui.binary_item = XtVaCreateManagedWidget("Write binary data", xmToggleButtonWidgetClass, dialog, NULL);
#if defined(HAVE_NETCDF) || defined(HAVE_MFHDF)
        wui.netcdf_item = XtVaCreateManagedWidget("Write netcdf data", xmToggleButtonWidgetClass, dialog, NULL);
#endif
...
void create_block_popup(Widget w, XtPointer client_data, XtPointer call_data) {
    ...
    if (block_dialog == NULL) {
        ...
        XtAddCallback(block_dialog, XmNhelpCallback, (XtCallbackProc)HelpCB, (XtPointer) "file.html#readblock");
