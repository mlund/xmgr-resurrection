...
void create_eblock_frame(Widget w, XtPointer client_data, XtPointer call_data) {
    ...
    if (eblock_frame == NULL) {
        ...
        eblock_ncols_item = XtVaCreateManagedWidget("tmp", xmLabelWidgetClass, eblock_panel, NULL);

        rc = XtVaCreateWidget("rc", xmRowColumnWidgetClass, eblock_panel, XmNpacking, XmPACK_COLUMN, XmNnumColumns, 9,
        ...
        eblock_type_choice_item =
            CreatePanelChoice(rc, " ", 14, "XY", "XY DX", "XY DY", "XY DX1 DX2", "XY DY1 DY2", "XY DX DY", "XY Z",
                              "XY HILO", "XY R", "XY BOX", "Rawspice", "XY BOXPLOT", "POLY", NULL, 0);
        for (i = 0; i < 12; i++) {
            XtAddCallback(eblock_type_choice_item[2 + i], XmNactivateCallback, (XtCallbackProc)eblock_type_notify_proc,
        ...
        XtAddCallback(buts[0], XmNactivateCallback, (XtCallbackProc)eblock_accept_notify_proc, (XtPointer)0);
        XtAddCallback(buts[1], XmNactivateCallback, (XtCallbackProc)destroy_dialog, (XtPointer)eblock_frame);
