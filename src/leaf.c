 ...
 */
#ifndef LEAF_PRSTR
char leaf_prstr[128] = "cat >xmgr.leaf <";
...
#endif
#ifndef LEAF_SUFFIX
char leaf_suffix[128] = ".leaf";

#else
char leaf_suffix[128] = LEAF_SUFFIX;
...
static char* leaf_fname;
static int orientflag = PAGE_PORTRAIT;
static int styles[5] = {0, 5, 4, 1, 2};
static int pattern[16] = {5, 10, 9, 8, 3, 2, 1, 4, 5, 11, 10, 9, 8, 3, 2, 1};

static char* fontlist[] = {"wst:timsps\0",  "wst:timsps\0", "wst:timsps\0", "wst:timsps\0",
                           "wst:helvps\0",  "wst:helvps\0", "wst:helvps\0", "wst:helvps\0",
                           "grk:dutchbs\0", "sym:clas\0",   "sym:clas\0"};
static char* fonttypelist[] = {" \0", "b\0", "i\0", "bi\0", " \0", "b\0", "i\0", "bi\0", " \0", " \0", " \0"};
...
static void stroke(void) {
    ...
    if (leaf_pathlength > 1) {
        fprintf(leafout, "(p8,1,0,,%d,%d,127\n", leafpattern, leafcolor);
        fprintf(leafout, "  (g9,1,0\n");
        fprintf(leafout, "    (g9,1,0\n");
        for (i = 1; i < leaf_pathlength; i++) {
            fprintf(leafout, "      (v7,1,0,%6.3f,%6.3f,%6.3f,%6.3f,%d,0,%d,%d)\n", (double)xpoints[i - 1] * 0.001,
                    (double)(leafymax - ypoints[i - 1]) * 0.001, (double)xpoints[i] * 0.001,
                    (double)(leafymax - ypoints[i]) * 0.001, leafcolor, leaflinewidth, styles[leaflinestyle - 1]);
        } /* for i */
        fprintf(leafout, ")))\n");
        xpoints[0] = xpoints[--leaf_pathlength];
        ypoints[0] = ypoints[leaf_pathlength];
        leaf_pathlength = 1;
...
void otherdefs(void) {
    ...
    for (i = 0; i <= MAXCOLOR; i++) {
        fprintf(leafout, "C%d =  %5.1f, %5.1f, %5.1f, 0.0,\n", i, (float)((255.0 - (float)red[i]) * 0.3922),
                (float)((255.0 - (float)green[i]) * 0.3922), (float)((255.0 - (float)blue[i]) * 0.3922));
    } /* for ncolors */
    fprintf(leafout, ">\n");
    fprintf(leafout, "\n");
    fprintf(leafout, "\n");
...
int leafsetmode(int mode) {
    ...
    if (mode % 2) {
        if (!ptofile) {
            ...
            leaf_fname = mktemp(tbuf);
        } else {
            leaf_fname = printstr;
        }
        if ((leafout = fopen(leaf_fname, "w")) == NULL) {
            return 0;
        }
    }
    devoffsx = devoffsy = 0;
    ...
    case 4:
        ...
        fclose(leafout);
        if (!ptofile) {
            sprintf(sysbuf, "%s %s", leaf_prstr, leaf_fname);
            system(sysbuf);
            unlink(leaf_fname);
        }
        orientflag = PAGE_PORTRAIT;
...
void drawleaf(int x2, int y2, int mode) {
    ...
    xpoints[leaf_pathlength] = xtmp;
    ypoints[leaf_pathlength++] = ytmp;
    x_current = xtmp;
    y_current = ytmp;
}

int xconvleaf(double x) { return ((int)(leafxmin + leafdx * xconv(x))); }

int yconvleaf(double y) { return ((int)(leafymin + leafdy * yconv(y))); }
...
void leafsetfont(int n) { leaffont = n; }

void leafsetfontsize(double size) { leaffontsize = (int)(size * 15); }

void dispstrleaf(int x, int y, int rot, char* s, int just, int fudge, char underline, int* leafsavefontsize,
                 int* leafsavefonttype) {
    ...
    *leafsavefonttype = leaffont;
    ...
    if (strlen(s) > 0) {
        if ((rot == 0) || (rot == 90) || (rot == 180) || (rot == 270)) {
            if (fudge) {
                fprintf(leafout, "   (t14,1,0,%f6.3,%f6.3,%d,%d,0,%d,%c,%s%d%s,", oldx * 0.001,
                        (leafymax - (oldy - leaffontsize / 216.0 * 1000.0)) * 0.001, leafjust[just], leafcolor, -rot,
                        underline, fontlist[leaffont], leaffontsize, fonttypelist[leaffont]);
            } else {
                fprintf(leafout, "   (t14,1,0,%f6.3,%f6.3,%d,%d,0,%d,%c,%s%d%s,", oldx * 0.001,
                        (leafymax - oldy) * 0.001, leafjust[just], leafcolor, -rot, underline, fontlist[leaffont],
                        leaffontsize, fonttypelist[leaffont]);
            }
        }
        /* if rotrad */
        else {
            if (fudge) {
                fprintf(leafout, "   (o4,1,0,%f6.3,%f6.3,%f6.3,%s%d%s,", x * 0.001,
                        (leafymax - (y - leaffontsize / 216.0 * 1000.0)) * 0.001, -rotrad, fontlist[leaffont],
                        leaffontsize, fonttypelist[leaffont]);
            } else {
                fprintf(leafout, "   (o4,1,0,%f6.3,%f6.3,%f6.3,%s%d%s,", x * 0.001, (leafymax - y) * 0.001, -rotrad,
...
void putleaf(char* s) {
    int i, slen = strlen(s);
    double saves = leaffontsize / 15.0;
    ...
    if (n) {
        ...
        for (i = 1; i < n; i++) {
            fprintf(leafout, "      (v7,1,0,%6.3f,%6.3f,%6.3f,%6.3f,7,127,%d,%d)\n", (double)px[i - 1] * 0.001,
                    (double)(leafymax - py[i - 1]) * 0.001, (double)px[i] * 0.001, (double)(leafymax - py[i]) * 0.001,
...
void leafdrawarc(int x, int y, int r) {
    ...
    fprintf(leafout, " %6.3f,%6.3f,%6.3f,%6.3f,%6.3f,%6.3f,\n", (x - r) * 0.001, (leafymax - y - r) * 0.001,
            (x + r) * 0.001, (leafymax - y - r) * 0.001, (x - r) * 0.001, (leafymax - y + r) * 0.001);
    fprintf(leafout, "  %d,127,%d,%d,0,%d,%d)\n", leafcolor, leafpattern, leafcolor, leaflinewidth,
            styles[leaflinestyle - 1]);
    leaf_pathlength = 0;
}
...
void leafdrawellipse(int x, int y, int xm, int ym) {
    ...
    fprintf(leafout, " %6.3f,%6.3f,%6.3f,%6.3f,%6.3f,%6.3f,\n", (x - xm) * 0.001, (leafymax - y - ym) * 0.001,
            (x + xm) * 0.001, (leafymax - y - ym) * 0.001, (x - xm) * 0.001, (leafymax - y + ym) * 0.001);
    fprintf(leafout, "  %d,127,%d,%d,0,%d,%d)\n", leafcolor, leafpattern, leafcolor, leaflinewidth,
            styles[leaflinestyle - 1]);
    leaf_pathlength = 0;
}
...
void leaffillellipse(int x, int y, int xm, int ym) {
    ...
    fprintf(leafout, " %6.3f,%6.3f,%6.3f,%6.3f,%6.3f,%6.3f,\n", (x - xm) * 0.001, (leafymax - y - xm) * 0.001,
            (x + xm) * 0.001, (leafymax - y - xm) * 0.001, (x - xm) * 0.001, (leafymax - y + xm) * 0.001);
    fprintf(leafout, "  %d,0,%d,%d,0,%d,%d)\n", leafcolor, leafpattern, leafcolor, leaflinewidth,
            styles[leaflinestyle - 1]);
    leaf_pathlength = 0;
}

void leafleavegraphics(void) { leafsetmode(leafdmode + 1); }
...
int leafinitgraphics(int dmode) {
    ...
    */
    ...
    devfillellipsecolour = leaffillellipse;
    devfill = leaffill;
...
static int strchkslash(char* s) {
    ...
    if (s[0] == '\\') {
        ...
        if (isoneof(st[0], "0123456789xsS+-NbuU"))
            for (i = 2; i <= slen; i++)
                s[i - 2] = s[i];
    }
    return (flag);
...
static void newcoord(char* s, int* x, int* y, int rot, int flag, int fontsize) {
    ...
    if (strlen(s) > 0) {
        ...
        default:
            /* error */
            errmsg("Internal error, newcoord called with wrong argument");
            facux = -1;
            facuy = 0;
            facdx = 1;
            facdy = 0;
...
static int strsplit(char* s1, char* s2) {
    ...
    if (ls1 > 1) {
        for (i = 0; i < ls1; i++) {
            ...
            if (s1[i] == '\\') {
                if (isoneof(s1[i + 1], "0123456789xsS+-NbuU")) {
                    for (j = i; j < ls1; j++)
                        s2[j - i] = s1[j];
                    s1[i] = '\0';
                    s2[ls1 - i] = '\0';
