    ...
    if (l.box == TRUE) { /* compute a bounding box for the legend */
        for (i = 0; i < maxplot; i++) {
            if (isactive_set(gno, i) && (g[gno].p[i].lstr[0])) {
                get_graph_plotarr(gno, i, &p);
                putlegend(j, 1, l.len, l.vgap, p.symsize, tmpx, tmpy, sy, ly, cy, wy, g[gno].p[i].lstr, p.symfill, -1,
                          -1, -1);
                j++;
            }
        }
        ...
        putlegendrect(l.boxfill, l.boxfillusing == CLRFILLED, l.boxfillcolor, l.boxfillpat, l.boxlcolor, l.boxlinew,
                      l.boxlines, g[gno].l.vgap);
    }
    j = 0;
    for (i = 0; i < maxplot; i++) {
        if (isactive_set(gno, i) && (g[gno].p[i].lstr[0])) {
            ...
            case GRAPH_STACKEDBAR:
            case GRAPH_STACKEDHBAR:
                putbarlegend(j, 0, l.len, l.vgap, p.symsize, tmpx, tmpy, sy, ly, cy, wy, g[gno].p[i].lstr, p.symfill,
                             p.fill, p.fillusing == PTNFILLED, p.fillusing == PTNFILLED ? p.fillpattern : p.fillcolor);
                break;
            default:
                putlegend(j, 0, l.len, l.vgap, p.symsize, tmpx, tmpy, sy, ly, cy, wy, g[gno].p[i].lstr, p.symfill,
                          p.symcolor, p.symlinew, p.symlines);
                break;
            }
            j++;
        }
...
void draw_ellipse(int gno, int i) {
    ...
    if (b.active) {
        ...
        if (b.fill == CLRFILLED || b.fill == PTNFILLED) {
            ...
            if (b.loctype == COORD_WORLD) {
                my_ellipse((b.x1 + b.x2) / 2, (b.y1 + b.y2) / 2, fabs(b.x1 - b.x2) / 2, fabs(b.y1 - b.y2) / 2, filling);
            } else {
                view2world(b.x1, b.y1, &xtmp1, &ytmp1);
                view2world(b.x2, b.y2, &xtmp2, &ytmp2);
                my_ellipse((xtmp1 + xtmp2) / 2, (ytmp1 + ytmp2) / 2, fabs(xtmp1 - xtmp2) / 2, fabs(ytmp1 - ytmp2) / 2,
                           filling);
            }
        }
