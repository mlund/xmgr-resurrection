...
void XRotSetMagnification(m) float m;
{
    if (m > 0.)
        style.magnify = m;
}
...
void XRotSetBoundingBoxPad(p) int p;
{
    if (p >= 0)
        style.bbx_pad = p;
}
...
int x, y;
char* str;
{ return (XRotPaintAlignedString(dpy, font, angle, drawable, gc, x, y, str, NONE, 0)); }
...
int x, y;
char* str;
{ return (XRotPaintAlignedString(dpy, font, angle, drawable, gc, x, y, str, NONE, 1)); }
...
char* text;
int align;
{ return (XRotPaintAlignedString(dpy, font, angle, drawable, gc, x, y, text, align, 0)); }
...
int XRotDrawAlignedImageString(dpy, font, angle, drawable, gc, x, y, text, align)
Display* dpy;
XFontStruct* font;
float angle;
Drawable drawable;
...
char* text;
int align;
{ return (XRotPaintAlignedString(dpy, font, angle, drawable, gc, x, y, text, align, 1)); }
...
static int XRotPaintAlignedString(dpy, font, angle, drawable, gc, x, y, text, align, bg)
Display* dpy;
XFontStruct* font;
float angle;
Drawable drawable;
...
int bg;
{
    ...
    if (bg) {
        ...
        for (i = 0; i < 4 * item->nl; i++) {
            xpoints[i].x =
                (float)x + ((item->corners_x[i] - hot_x) * cos_angle + (item->corners_y[i] + hot_y) * sin_angle);
            xpoints[i].y =
                (float)y + (-(item->corners_x[i] - hot_x) * sin_angle + (item->corners_y[i] + hot_y) * cos_angle);
        }
        ...
        XFillPolygon(dpy, drawable, my_gc, xpoints, 4 * item->nl, Nonconvex, CoordModeOrigin);
...
#ifndef X11R3
    {
        ...
        if (XGetGCValues(dpy, gc,
            ...
            if ((values.fill_style == FillStippled || values.fill_style == FillOpaqueStippled) && !bg) {
                ...
                new_bitmap = XCreatePixmap(dpy, drawable, item->cols_out, item->rows_out, 1);
                ...
                XSetTSOrigin(dpy, depth_one_gc, values.ts_x_origin - xp, values.ts_y_origin - yp);
                ...
                XFillRectangle(dpy, new_bitmap, depth_one_gc, 0, 0, item->cols_out, item->rows_out);

                /* set stipple origin back to normal */
                XSetTSOrigin(dpy, depth_one_gc, 0, 0);

                /* this will contain an inverse copy of the rotated text */
                inverse = XCreatePixmap(dpy, drawable, item->cols_out, item->rows_out, 1);

                /* invert text */
                XSetFillStyle(dpy, depth_one_gc, FillSolid);
                XSetFunction(dpy, depth_one_gc, GXcopyInverted);
                XCopyArea(dpy, item->bitmap, inverse, depth_one_gc, 0, 0, item->cols_out, item->rows_out, 0, 0);
...
static int XRotDrawHorizontalString(dpy, font, drawable, gc, x, y, text, align, bg)
Display* dpy;
XFontStruct* font;
Drawable drawable;
GC gc;
...
int align;
{
    ...
    while (i1 && !item) {
        ...
        if (strcmp(text, i1->text) == 0 && fabs(angle - i1->angle) < 0.00001 && style.magnify == i1->magnify &&
            (i1->nl == 1 || ((align == 0) ? 9 : (align - 1)) % 3 == ((i1->align == 0) ? 9 : (i1->align - 1)) % 3)) {
...
#ifdef CACHE_XIMAGES
    {
        ...
        XPutImage(dpy, item->bitmap, depth_one_gc, item->ximage, 0, 0, 0, 0, item->cols_out, item->rows_out);
...
int align;
{
    ...
    item->cols_out = fabs((float)item->rows_in * sin_angle) + fabs((float)item->cols_in * cos_angle) + 0.99999 + 2;

    item->rows_out = fabs((float)item->rows_in * cos_angle) + fabs((float)item->cols_in * sin_angle) + 0.99999 + 2;
    ...
    /* make the text bitmap from XImage */
    XPutImage(dpy, item->bitmap, font_gc, item->ximage, 0, 0, 0, 0, item->cols_out, item->rows_out);
...
static void XRotAddToLinkedList(dpy, item) Display* dpy;
RotatedTextItem* item;
{
...
static void XRotFreeTextItem(dpy, item) Display* dpy;
RotatedTextItem* item;
{
    free(item->text);
...
int align;
{
    ...
    for (i = 0; i < 5; i++) {
        xp_out[i].x = (float)x + (((float)xp_in[i].x - hot_x) * cos_angle + ((float)xp_in[i].y + hot_y) * sin_angle);
        xp_out[i].y = (float)y + (-((float)xp_in[i].x - hot_x) * sin_angle + ((float)xp_in[i].y + hot_y) * cos_angle);
    }
