...
int includ(int np, int nrbar, double w, double* xrow, double y, double* d, double* rbar, double* thetab,
           double* sserr) {
    int ier = 0, i, k, nextr;
...
int clear(int np, int nrbar, double* d, double* rbar, double* thetab, double* sserr)
...
int regcf(int np, int nrbar, double* d, double* rbar, double* thetab, double* tol, double* beta, int nreq) {
    int ier = 0, i, j, nextr;
...
int sing(int np, int nrbar, double* d, double* rbar, double* thetab, double* sserr, double* tol, int* lindep)
    ...
    for (col = 1; col <= np; ++col) {
        ...
        if (work[col] < temp) {
            ...
            if (col < np) {
                np2 = np - col;
                pos2 = pos + np2 + 1;
                ier = includ(np2, np2 * (np2 - 1) / 2, d[col], rbar + pos + 1, thetab[col], d + col + 1, rbar + pos2,
                             thetab + col + 1, sserr);
            } else {
...
int cov(int np, int nrbar, double* d, double* rbar, int nreq, double* rinv, double* var, double* covmat, int dimcov,
        double* sterr)
...
int pcorr(int np, int nrbar, double* d, double* rbar, double* thetab, double* sserr, int in, double* cormat, int dimc,
          double* ycorr)
...
void cor(int np, double* d, double* rbar, double* thetab, double* sserr, double* work, double* cormat, double* ycorr)
...
int vmove(int np, int nrbar, int* vorder, double* d, double* rbar, double* thetab, double* rss, int from, int to,
          double* tol)
...
int reordr(int np, int nrbar, int* vorder, double* d, double* rbar, double* thetab, double* rss, double* tol, int* list,
           int n, int pos1)
   ...
   position POS1 are not moved.  Auxiliary routine called: VMOVE */
{
    ...
    do {
        ...
        for (j = 1; j <= n; j++)
            if (l == list[j]) {
                if (i > next)
                    vmove(np, nrbar, vorder + 1, d + 1, rbar + 1, thetab + 1, rss + 1, i, next, tol + 1);
                if (++next >= n + pos1)
                    return 0;
                break;
            }
...
int hdiag(double* xrow, int np, int nrbar, double* d, double* rbar, double* tol, int nreq, double* hii)
