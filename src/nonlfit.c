...
int lmdif_drv(U_fp fcn, integer m, integer n, doublereal* x, doublereal* fvec, doublereal* tol, integer* iwa,
...
void a_to_parms(double* a) {
    ...
    for (i = 0; i < nonl_opts.parnum; i++) {
        if (nonl_parms[i].constr) {
            ...
            nonl_parms[i].value =
                (nonl_parms[i].min + nonl_parms[i].max) / 2.0 + (nonl_parms[i].max - nonl_parms[i].min) / 2.0 * t;
        } else {
...
int do_nonlfit(int gno, int setno, int nsteps) {
    ...
    info =
        lmdif_drv((U_fp)fcn, (integer)n, (integer)parnum, a, fvec, &nonl_opts.tolerance, iwa, wa, lwa, (integer)nsteps);
...
int lmdif_drv(U_fp fcn, integer m, integer n, doublereal* x, doublereal* fvec, doublereal* tol, integer* iwa,
    ...
    extern int lmdif_(U_fp, integer*, integer*, doublereal*, doublereal*, doublereal*, doublereal*, doublereal*,
                      integer*, doublereal*, doublereal*, integer*, doublereal*, integer*, integer*, integer*,
                      doublereal*, integer*, integer*, doublereal*, doublereal*, doublereal*, doublereal*, doublereal*);
    ...
    lmdif_((U_fp)fcn, &m, &n, &x[1], &fvec[1], &ftol, &xtol, &gtol, &maxfev, &epsfcn, &wa[1], &mode, &factor, &nprint,
           &info, &nfev, &wa[mp5n + 1], &m, &iwa[1], &wa[n + 1], &wa[(n << 1) + 1], &wa[n * 3 + 1], &wa[(n << 2) + 1],
           &wa[n * 5 + 1]);
