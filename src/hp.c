...
void putstrhp(char* s) { fprintf(hpout, "%s", s); }
...
void hpsetfont(int n) { hselectfont(hpfont = n); }
...
/* only one line width */
int hpsetlinewidth(int c) { return c; }
...
int hpsetpat(int pat) { return (hpfillpat = pat % 7); }
...
void hpleavegraphics(void) { hpsetmode(hpdmode + 1); }
