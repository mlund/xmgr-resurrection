/* $Id: framewin.c,v 1.1 1995/04/13 16:25:49 pturner Exp pturner $
 *
 * frame Panel
 *
 */
...
void update_frame_items(int gno) {
    if (frame_frame) {
        XmToggleButtonSetState(frame_frameactive_choice_item, g[gno].f.active == TRUE, False);
        ...
        XmToggleButtonSetState(frame_fillbg_choice_item, g[gno].f.fillbg == TRUE, False);
...
void create_frame_frame(Widget w, XtPointer client_data, XtPointer call_data) {
    ...
    if (frame_frame == NULL) {
        ...
        frame_frameactive_choice_item = XmCreateToggleButton(frame_panel, "Display graph frame", NULL, 0);
        XtManageChild(frame_frameactive_choice_item);

        rc = XtVaCreateWidget("rc", xmRowColumnWidgetClass, frame_panel, XmNpacking, XmPACK_COLUMN, XmNnumColumns, 5,
        ...
        frame_framestyle_choice_item = CreatePanelChoice(rc, " ", 7, "Closed", "Half open", "Break top", "Break bottom",
                                                         "Break left", "Break right", NULL, NULL);
        ...
        frame_linew_choice_item =
            CreatePanelChoice(rc, " ", 10, "1", "2", "3", "4", "5", "6", "7", "8", "9", NULL, NULL);

        XtVaCreateManagedWidget("Line style:", xmLabelWidgetClass, rc, NULL);
        frame_lines_choice_item = CreatePanelChoice(rc, " ", 6, "Solid line", "Dotted line", "Dashed line",
                                                    "Long Dashed", "Dot-dashed", NULL, NULL);

        frame_fillbg_choice_item = XmCreateToggleButton(rc, "Fill graph frame", NULL, 0);
        XtManageChild(frame_fillbg_choice_item);

        frame_bgcolor_choice_item = CreateColorChoice(rc, "Fill color:", 0);
        XtManageChild(rc);

        frame_applyto_choice_item =
            CreatePanelChoice(frame_panel, "Apply to:", 3, "Current graph", "All active graphs", NULL, NULL);

        XtVaCreateManagedWidget("sep", xmSeparatorWidgetClass, frame_panel, NULL);

        CreateCommandButtons(frame_panel, 2, buts, label1);
        XtAddCallback(buts[0], XmNactivateCallback, (XtCallbackProc)frame_define_notify_proc, (XtPointer)NULL);
        XtAddCallback(buts[1], XmNactivateCallback, (XtCallbackProc)destroy_dialog, (XtPointer)frame_frame);
...
static void frame_define_notify_proc(Widget w, XtPointer client_data, XtPointer call_data) {
    ...
    for (i = ming; i <= maxg; i++) {
        if (isactive_graph(i)) {
            g[i].f.active = XmToggleButtonGetState(frame_frameactive_choice_item) ? TRUE : FALSE;
            ...
            g[i].f.fillbg = XmToggleButtonGetState(frame_fillbg_choice_item) ? TRUE : FALSE;
...
void UpdateFrameWinColors(unsigned color) {
    UpdateColorChoice(frame_color_choice_item, color);
    UpdateColorChoice(frame_bgcolor_choice_item, color);
}
