...
void clear_world_stack(void) {
    g[cg].ws_top = 1;
    g[cg].curw = 0;
    g[cg].ws[0].w.xg1 = g[cg].ws[0].w.xg2 = g[cg].ws[0].w.yg1 = g[cg].ws[0].w.yg2 = 0;
#ifndef NONE_GUI
    set_stack_message();
#endif
}
...
void add_world(int gno, double x1, double x2, double y1, double y2, double t1, double t2, double u1, double u2) {
    ...
    if (g[cg].ws[0].w.xg1 == 0. && g[cg].ws[0].w.xg2 == 0. && g[cg].ws[0].w.yg1 == 0. && g[cg].ws[0].w.yg2 == 0.) {
        g[gno].ws_top = 0;
    }
...
void make_format(int gno) {
    ...
    case 0:
        if (locfx < 3 && locfy < 3) {
            sprintf(locator_format, "G%%1d: %s = [%%.%d%s, %%.%d%s]", typestr[type], locpx, fchar[locfx], locpy,
                    fchar[locfy]);
        } else {
            locator_format[0] = 0;
        }
        break;
    case 2:
        locfx = locfx == FORMAT_DECIMAL ? 0 : locfx == FORMAT_EXPONENTIAL ? 1 : locfx == FORMAT_GENERAL ? 2 : 0;
    ...
    case 4:
        locfx = locfx == FORMAT_DECIMAL ? 0 : locfx == FORMAT_EXPONENTIAL ? 1 : locfx == FORMAT_GENERAL ? 2 : 0;
        locfy = locfy == FORMAT_DECIMAL ? 0 : locfy == FORMAT_EXPONENTIAL ? 1 : locfy == FORMAT_GENERAL ? 2 : 0;
        sprintf(locator_format, "G%%1d: %s = [%%.%d%s, %%.%d%s]", typestr[type], locpx, fchar[locfx], locpy,
                fchar[locfy]);
        break;
    case 5:
