/* $Id: monwin.c,v 1.1 1995/04/13 16:25:49 pturner Exp pturner $
 *
 * monitor Panel
 *
 */
...
void create_monitor_frame(Widget w, XtPointer client_data, XtPointer call_data) {
    ...
    if (mon_frame == NULL) {
        ...
        mon_log_item = XtVaCreateManagedWidget("Log", xmToggleButtonWidgetClass, rc, NULL);
        ...
        XtVaSetValues(fr, XmNtopAttachment, XmATTACH_FORM, XmNleftAttachment, XmATTACH_FORM, XmNrightAttachment,
...
static void clear_results(Widget w, XtPointer client_data, XtPointer call_data) { XmTextSetString(text_w, ""); }
...
static void create_wmon_frame(Widget w, XtPointer client_data, XtPointer call_data) {
    ...
    if (wmon_frame == NULL) {
        ...
        XtAddCallback(buts[0], XmNactivateCallback, (XtCallbackProc)wmon_apply_notify_proc, (XtPointer)0);
        XtAddCallback(buts[1], XmNactivateCallback, (XtCallbackProc)destroy_dialog, (XtPointer)wmon_frame);
